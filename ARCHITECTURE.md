...
6. **Perfis de performance por SO na criação.** O `osType` informado na criação (`linux` | `windows` | `other`) seleciona um perfil de domínio aplicado por um gerador de XML puro e testável (`backend/internal/hypervisor/libvirt/domainxml.go`, structs + `encoding/xml`, sem concatenação de string). O perfil linux/windows usa virtio-scsi com iothread, `cache='none'`, vhost multiqueue, `host-passthrough` (limita live migration entre CPUs diferentes — relevante na fase multi-host), canal do QEMU Guest Agent, memballoon off, timers e RNG. Windows ganha Hyper-V enlightenments e `localtime`/hypervclock; os enlightenments aplicados são **detectados pelas versões de QEMU/libvirt do host** (tabela conservadora em `domainxml.go`) — nunca uma lista fixa — e o efetivamente aplicado é reportado em `performanceProfile` na leitura da VM. VMs criadas antes dos perfis continuam funcionando (`osType`/`performanceProfile` nulos). UEFI/Secure Boot/TPM para Windows 11 ficam para depois; a estrutura de structs do gerador já comporta esses elementos.
